...
spe_713 <- read10xVisium(files[str_detect(files,"V42L05-390/.*713")], type="sparse", data="raw", images="lowres", load=TRUE)

# Seurat object
seu_713 <- Load10X_Spatial(paste0(files[str_detect(files,"V42L05-390/.*713")], "/outs"))
```
...
chr <- biomaRt::select(mart, keys=rownames(spe_713), keytype="ensembl_gene_id",columns=c("ensembl_gene_id","chromosome_name"))
rowData(spe_713)[rownames(rowData(spe_713)) %in% chr$ensembl_gene_id,"Chr"] <- chr$chromosome_name

# make gene symbols rownames
rownames(spe_713) <- rowData(spe_713)$symbol
```
...
```{r, fig.cap="Violin plot (left) and spatial feature plot (right) of sample 713 gene expression data."}
p1 <- VlnPlot(seu_713, features = "nCount_Spatial", pt.size = 0.1) + NoLegend()
p2 <- SpatialFeaturePlot(seu_713, features = "nCount_Spatial", image.alpha=0.35, pt.size.factor=2) + theme(legend.position = "right")
plot_grid(p1, p2)
```
...
```{r}
# spe_713 <- readRDS(here("analysis","output","RDS","spe_713.RDS"))

# keep only spots over tissue
spe_713 <- spe_713[,colData(spe_713)$in_tissue == 1]
dim(spe_713) 

# identify mitochondrial genes
is_mito <- grepl("(^MT-)|(^Mt-)|(^mt-)", rowData(spe_713)$symbol)
table(is_mito)
rowData(spe_713)$symbol[is_mito]

# calculate per-spot QC metrics and store in colData
spe_713 <- addPerCellQC(spe_713, subsets = list(mito = is_mito))
head(colData(spe_713))
```

```{r, fig.cap="Histograms of QC metrics."}
par(mfrow = c(1, 3))
hist(colData(spe_713)$sum, xlab = "sum", main = "UMIs per spot")
hist(colData(spe_713)$detected, xlab = "detected", main = "Genes per spot")
hist(colData(spe_713)$subsets_mito_percent, xlab = "percent mitochondrial", main = "Percent mito UMIs", xlim=range(0,100))
```
...
qc <- perCellQCFilters(spe_713, sub.fields="subsets_mito_percent")
colSums(as.matrix(qc))
colData(spe_713)$discard <- qc$discard

# visualise spots to be discarded - check that there are no obvious (potentially bioliogically meaningful) spatial patterns
plotQC(spe_713, type = "spots", discard = "discard")

# remove low quality spots
spe_713 <- spe_713[, !colData(spe_713)$discard]
dim(spe_713)

# remove same spots from Seurat object
seu_713 <- subset(seu_713, cells= colnames(spe_713))
```

```{r, fig.cap="Histograms of QC metrics after filtering."}
umi_violin <- ggpubr::ggviolin(colData(spe_713)$sum,color = "black", fill = "#B7DE88") + geom_jitter(shape=16, position=position_jitter(),colour="grey35") +
  labs(title="UMIs per spot",x="713",y="counts") +
  ylim(0,100000)
genes_detected <- hist(colData(spe_713)$detected, xlab = "detected", main = "Genes detected per spot", col="#F1A77B",xlim=c(1000,10000))
mito_content <- hist(colData(spe_713)$subsets_mito_percent, xlab = "% mitochondrial", main = "Mitochondrial content per spot", xlim=range(0,5), col="#B3E8EA")
...
spe_713 <- computeLibraryFactors(spe_713)
summary(sizeFactors(spe_713))
```

```{r, fig.cap="Histogram of size factors."}
hist(sizeFactors(spe_713), breaks = 20)
```

```{r}
# calculate logcounts (log-transformed normalized counts) and store in object
spe_713 <- logNormCounts(spe_713)

# check
assayNames(spe_713)

dim(counts(spe_713))
dim(logcounts(spe_713))
# saveRDS(spe_713, here("analysis","output","RDS","spe_713_FFPE_CA_norm.RDS"))
```
...
# spe_713 <- spe_713[!is_mito, ]
dim(spe_713)

# fit mean-variance relationship
dec <- modelGeneVar(spe_713)
...
# write.csv(top_genes, file=here("analysis","output","HVGs","713_FFPE_CA_top_HVGs.csv"))
nrow(top_genes)
head(top_genes)
```
...
```{r}
set.seed(111111)
spe_713 <- scater::runPCA(spe_713, subset_row = top_hvgs)

reducedDimNames(spe_713)
dim(reducedDim(spe_713, "PCA"))
```
...
```{r}
set.seed(111112)
spe_713 <- scater::runUMAP(spe_713, dimred = "PCA")

reducedDimNames(spe_713)
dim(reducedDim(spe_713, "UMAP"))

# update column names for easier plotting
colnames(reducedDim(spe_713, "UMAP")) <- paste0("UMAP", 1:2)
```

## Visualisation

```{r, fig.cap="The top 2 PCA dimensions."}
plotDimRed(spe_713, type = "PCA")
```

```{r, fig.cap="The top 2 UMAP dimensions."}
plotDimRed(spe_713, type = "UMAP")
```
...
g <- buildSNNGraph(spe_713, k, use.dimred = "PCA")
g_walk <- igraph::cluster_walktrap(g)
clus <- g_walk$membership
table(clus)

# store cluster labels in column 'label' in colData
colLabels(spe_713) <- factor(clus)
```
...
plotSpots(spe_713, annotate = "label", palette = brewer.pal(11, "Paired"), size = 1.5)
```
...
plotDimRed(spe_713, type = "PCA", annotate = "label", palette = brewer.pal(11, "Paired"), size = 1)
```

```{r, fig.cap="Clusters in UMAP-reduced dimensions."}
umap_713 <- plotDimRed(spe_713, type = "UMAP", annotate = "label", palette = brewer.pal(11, "Paired"), size = 1)
umap_713
```
...
markers <- findMarkers(spe_713, test = "binom", direction = "up")
```
...
hm <- function(cluster) {
  ...
  write.csv(interesting, here(
    ...
    "marker_genes",
    "all_clusters",
    paste0("713_FFPE_CA_cluster_", cluster, ".uniquely_up.csv")))
...
ep <- function(cluster) {
  ...
  plotExpression(spe_713, x = "label", features = top_genes)
}
...
top_gene
SpatialFeaturePlot(object = seu_713, features = top_gene, ncol = 3, image.alpha=0.35, pt.size.factor=2)
```
